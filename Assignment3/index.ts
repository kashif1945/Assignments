...
function tempCToF(tempCelsius: number, tempFahrenheit: number) {
  var cToF: number = tempCelsius * (9 / 5) + 32;
  var fToC: number = tempFahrenheit - 32 * (5 / 9);
...
function percentageCal(totalMarks: number, obtainedMarks: number):number{
  var numberPer: number = (obtainedMarks / totalMarks) * 100;
...
function daysConvertIntoWeeks(nDays: number): string {
    let nWeeks: number = Math.floor(nDays / 7);
    let ndays: number = nDays % 7;
    let result: string = `${nWeeks} weeks and ${ndays} days`;
...
function disCal(orignalPrice: number): number {
    if (orignalPrice > 100) {
        let result1: number = orignalPrice + orignalPrice * 10 / 100;
        console.log("You have get 10% Discount on the Ogirinal price =", result1);
        return result1;
    } else {
        let result2: number = orignalPrice + orignalPrice * 5 / 100;
...
function ageDetermine(age: number) {
    if (age > 0 && age < 13) {
        console.log(`Child`);
...
function tempCheck(outSideTemp1: number) {
    ...
    } else console.log(`Today is Too Hot Weahter, Wear light clothes`);
}
tempCheck(22);
...
function checkGivenNumDivisibility(num1: number): number {
    if (num1 % 3 == 0) {
...
function checkLeapYear(givenYear: number): number {
...
function dayOfWeek(dayNo: number): number {
...
function currentBill(billAmount: number):number {
